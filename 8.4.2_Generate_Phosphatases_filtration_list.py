...
Folder = "Results/Categories/6.Phosphatases"

Folder_out = "Lists/Filtration lists"

# Files #

File = "Sample3_Male_Phosphatases.csv"

File_out = "Filtration_list_Phosphatases.csv"
...
# Read Phosphatase file #
with open(os.path.join(Folder,File),'r') as read:
    next(read)
    for line in read:
        line = line.strip().split(";")
        if not line[0] in Phosphatase_genes:
...
with open(os.path.join(Folder_out,File_out),'w+',encoding="utf-8") as out:
    ...
    for key in Phosphatase_genes:
        ## progress info ##
        counter += 1
        print("{} - {}/{}".format(key,counter,len(Phosphatase_genes)))
        ### Get lookup values ###
        ## Initialize ##
        lookup = []
